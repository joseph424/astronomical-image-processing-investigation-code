...
plt.title("Number count plot",fontsize=16)
plt.xlim([8,22])
plt.ylim([0.5,3.5])
plt.errorbar(mag_points,np.log10(no_galaxies),yerr=(error_lower, error_upper),fmt='.',capsize=3, color='k')
plt.grid()
plt.savefig("Number count plot.jpg", dpi=300,bbox_inches='tight')
plt.show()
...
plt.grid()
plt.title('Fraction of fake sources detected',fontsize=16)
plt.xlabel('Magnitude',fontsize=12)
plt.ylabel('Fraction detected',fontsize=12)
plt.xlim([14,18])
plt.savefig("Fraction of fake sources plot.jpg",dpi=300,bbox_inches='tight')
plt.show()